      ...
      >
        <VStack space={3} p={4}>
          ...
          <HStack space={2} alignItems="center">
            ...
            <Text fontSize="xs" color="coolGray.500">
              {t('ADOPTION_REQUESTS.ADOPTER.CARD.REQUESTED_ON')}: {formatDate(item.createdAt)}
            </Text>
          </HStack>
        </VStack>
      </Box>
  ...
  if (!requests || requests.length === 0) {
    return (
      <Center flex={1} px={6}>
        ...
        <Text fontSize="lg" fontWeight="bold" textAlign="center" mb={2}>
          {t('ADOPTION_REQUESTS.ADOPTER.EMPTY_LIST')}
        </Text>
        <Text fontSize="sm" color="coolGray.500" textAlign="center">
          {t('ADOPTION_REQUESTS.ADOPTER.EMPTY_LIST_DESCRIPTION')}
        </Text>
      </Center>
    );
  }
