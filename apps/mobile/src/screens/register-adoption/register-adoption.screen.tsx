...
import { Avatar, Button, Headline, RadioButton, Subheading, TextInput } from 'react-native-paper';
  ...
  return (
    <Container>
      ...
      <Form>
        ...
        <RNSlider />
        <Subheading style={{ color: theme.colors.primary }}>1 {t('YEAR')}</Subheading>
        <RadioButton.Group onValueChange={(value) => setChecked(value)} value={checked}>
          <Subheading>{t('REGISTER_ADOPTION.FORM.SEX.LABEL')}</Subheading>
          <RadioButton.Item label={t('REGISTER_ADOPTION.FORM.SEX.MALE')} value="first" />
          <RadioButton.Item label={t('REGISTER_ADOPTION.FORM.SEX.FEMALE')} value="second" />
        </RadioButton.Group>
